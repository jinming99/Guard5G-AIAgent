 ...
 */
 ...
 #include <pthread.h>
 #include <errno.h>
 #include <json-c/json.h>  // For JSON export (install libjson-c-dev)
 
 // === NEW: Global variables for config socket ===
 #define ORAN_CONFIG_SOCKET_PATH "/tmp/oai_oran_cfg.sock"
 static int config_socket_fd = -1;
 static pthread_t config_server_thread;
 static struct xran_fh_config *global_fh_config = NULL;  // Store config for socket access
 
 // === NEW: Function prototypes ===
 static char* oran_config_to_json(const struct xran_fh_config *config);
 static void* config_socket_server(void *arg);
 static void start_config_socket_server(void);
 ...
 static char* oran_config_to_json(const struct xran_fh_config *config) {
   ...
   json_object_object_add(root, "dpdk_port", 
       json_object_new_int(config->dpdk_port));
   json_object_object_add(root, "sector_id", 
       json_object_new_int(config->sector_id));
   json_object_object_add(root, "mtu", 
       json_object_new_int(9600));  // Default MTU value
   
   // Add timing configuration
   struct json_object *timing = json_object_new_object();
   json_object_object_add(timing, "t1a_min_cp_dl", 
       json_object_new_int(config->T1a_min_cp_dl));
   json_object_object_add(timing, "t1a_max_cp_dl", 
       json_object_new_int(config->T1a_max_cp_dl));
   json_object_object_add(timing, "t2a_min_cp_dl", 
       json_object_new_int(config->T2a_min_cp_dl));
   json_object_object_add(timing, "t2a_max_cp_dl", 
       json_object_new_int(config->T2a_max_cp_dl));
   json_object_object_add(root, "timing", timing);
   
   // Add radio parameters
   struct json_object *radio = json_object_new_object();
   json_object_object_add(radio, "num_cc", 
       json_object_new_int(config->nCC));
   json_object_object_add(radio, "num_antennas_tx", 
       json_object_new_int(config->neAxc));
   json_object_object_add(radio, "num_antennas_rx", 
       json_object_new_int(config->neAxcUl));
   json_object_object_add(radio, "compression", 
       json_object_new_int(config->ru_conf.compMeth));
   json_object_object_add(root, "radio", radio);
   
   // Add RU configuration
   struct json_object *ru = json_object_new_object();
   json_object_object_add(ru, "iq_width", 
       json_object_new_int(config->ru_conf.iqWidth));
   json_object_object_add(ru, "iq_width_prach", 
       json_object_new_int(config->ru_conf.iqWidth_PRACH));
   json_object_object_add(ru, "fft_size", 
       json_object_new_int(config->ru_conf.fftSize));
   json_object_object_add(root, "ru_config", ru);
 ...
 // === NEW: Socket server implementation ===
 static void* config_socket_server(void *arg) {
   ...
   strncpy(addr.sun_path, ORAN_CONFIG_SOCKET_PATH, sizeof(addr.sun_path) - 1);
   
   unlink(ORAN_CONFIG_SOCKET_PATH);  // Remove old socket if exists
   
   if (bind(config_socket_fd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
     printf("[ORAN] Failed to bind config socket\n");
     close(config_socket_fd);
     return NULL;
   }
   
   // Listen for connections
   if (listen(config_socket_fd, 5) < 0) {
     printf("[ORAN] Failed to listen on config socket\n");
     close(config_socket_fd);
     return NULL;
   }
   
   printf("[ORAN] Config socket server started at %s\n", ORAN_CONFIG_SOCKET_PATH);
   
   // Accept connections
   while (1) {
     client_fd = accept(config_socket_fd, NULL, NULL);
     if (client_fd < 0) {
       if (errno == EINTR) continue;
       printf("[ORAN] Failed to accept connection\n");
       break;
     }
     ...
     if (n > 0) {
       ...
       if (strncmp(buffer, "GET_CONFIG", 10) == 0) {
         // Send current config as JSON
         if (global_fh_config) {
           char *json = oran_config_to_json(global_fh_config);
         ...
         // Allow limited runtime updates (example)
         int new_mtu = atoi(buffer + 8);
         if (new_mtu >= 1500 && new_mtu <= 9600) {
           // Note: actual MTU update would require updating xran_fh_init
           send(client_fd, "OK\n", 3, 0);
           printf("[ORAN] MTU update requested to %d via socket\n", new_mtu);
         } else {
           send(client_fd, "ERROR: Invalid MTU\n", 19, 0);
         }
       }
       else if (strncmp(buffer, "SET_POWER ", 10) == 0) {
         // Adjust TX power (example)
         int new_power = atoi(buffer + 10);
         if (new_power >= -50 && new_power <= 30) {
           // Apply power adjustment (implementation specific)
           send(client_fd, "OK\n", 3, 0);
           printf("[ORAN] TX power adjusted to %d dBm\n", new_power);
         } else {
           send(client_fd, "ERROR: Invalid power\n", 21, 0);
         }
       }
       else {
 ...
 static void start_config_socket_server(void) {
   ...
   if (pthread_create(&config_server_thread, &attr, 
                     config_socket_server, NULL) != 0) {
     printf("[ORAN] Failed to create config socket server thread\n");
   }
 ...
 bool set_fh_init(struct xran_fh_init *fh_init)
 {
   ...
   // === NEW: Export initial config and start socket server ===
   // Note: This is called for xran_fh_init, but we want it for xran_fh_config
   // The actual export will happen in set_fh_config function
 ...
 bool set_fh_config(int ru_idx, int num_rus, const openair0_config_t *oai0, struct xran_fh_config *fh_config)
 {
   ...
   if (fp) {
     char *json = oran_config_to_json(fh_config);
     fprintf(fp, "%s\n", json);
     fclose(fp);
     free(json);
     printf("[ORAN] Config exported to /tmp/oai_oran_config.json\n");
   }
   
   // Start config socket server (only once, for first RU)
   static int socket_started = 0;
   if (!socket_started) {
     start_config_socket_server();
     socket_started = 1;
   }
